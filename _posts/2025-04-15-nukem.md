...
We remember that there was a MySQL instance running on the target let’s try to find credentials for it, actually we can do that running [linpeas.sh](http://linpeas.sh) for simplicity, if that won’t work we will switch to manual enumeration.
